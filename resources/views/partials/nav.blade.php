@php
    ...
    $navRole = session('livingcode_role', 'kasif');
    $accentText = $navRole === 'mimar' ? 'text-amethyst-200' : 'text-gold-200';
...
<nav id="navbar" class="navbar fixed top-0 left-0 right-0 z-50 px-4 sm:px-6 py-3" role="navigation" aria-label="Ana navigasyon">
    <div class="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center gap-8" role="menubar">
            <a href="{{ route('welcome') }}" class="nav-link {{ $currentRoute === 'welcome' ? $accentText : 'text-parchment-200 ' . $accentHover }} focus:outline-none focus:ring-2 {{ $accentRing }} focus:ring-offset-2 focus:ring-offset-dark-900 rounded" role="menuitem" @if($currentRoute === 'welcome') aria-current="page" @endif>Tanıtım</a>
            <a href="{{ route('welcome') }}#sss" class="nav-link text-parchment-200 {{ $accentHover }} focus:outline-none focus:ring-2 {{ $accentRing }} focus:ring-offset-2 focus:ring-offset-dark-900 rounded" role="menuitem">SSS</a>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <button id="mobile-toggle" class="md:hidden text-parchment-200 {{ $accentHover }} p-2 min-w-[44px] min-h-[44px] flex items-center justify-center focus:outline-none focus:ring-2 {{ $accentRing }} rounded" aria-label="Menüyü aç/kapat" aria-expanded="false" aria-controls="mobile-menu">
