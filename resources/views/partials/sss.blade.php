...
>
    <div class="max-w-4xl mx-auto section-sura-content !ml-auto !mr-auto">
        <div class="text-center mb-12">
            <span class="section-voice-label {{ $activeRole === 'mimar' ? 'text-amethyst-200/90' : 'text-gold-300' }} text-xs font-mono tracking-widest uppercase block mb-2 reveal">Bilgi Merkezi</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="faq-item rounded-xl border border-white/10 bg-white/[0.02] overflow-hidden reveal">
                <div class="faq-question flex items-center gap-4 p-5 cursor-pointer">
                    <span class="{{ $activeRole === 'mimar' ? 'text-amethyst-200/80' : 'text-gold-200/80' }} font-mono text-xs flex-shrink-0">{{ $num }}</span>
        ...
        <div class="text-center mt-12 reveal">
            <a href="mailto:{{ $contact['email'] ?? config('livingcode.contact.email') }}" class="{{ $activeRole === 'mimar' ? 'text-amethyst-200' : 'text-gold-200' }} text-sm hover:underline font-medium">
